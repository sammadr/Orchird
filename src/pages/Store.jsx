...
function Store() {
  ...
  const handleAddToCart = (product) => {
    ...
    const currentCart = JSON.parse(localStorage.getItem('orchirdCart') ?? '[]')
    ...
    localStorage.setItem('orchirdCart', JSON.stringify(currentCart))
            ...
            >
              <div className="relative p-4">
                ...
                <button
                  ...
                  className="absolute right-4 top-4 inline-flex h-12 w-12 items-center justify-center rounded-full border border-white/70 bg-linear-to-br from-white via-[#f3ebfa] to-[#e5f7ea] text-(--orchird-green-dark) shadow-[0_10px_22px_rgba(69,32,110,0.2)] transition hover:scale-105 hover:from-(--orchird-lilac) hover:to-white"
              ...
              <div className="p-5">
                ...
                <div className="mt-5 flex items-center justify-between gap-3">
                  <div className="inline-flex items-center overflow-hidden rounded-md border border-[#8556b1]">
                    <button
                      type="button"
                      onClick={() => updateQuantity(product.id, -1)}
                      className="inline-flex h-8 w-8 items-center justify-center bg-[#b889d7] text-xs text-white transition hover:bg-[#a770ce]"
                      aria-label={`Disminuir cantidad de ${product.name}`}
                    >
                      <FaMinus />
                    </button>
                    <span className="inline-flex h-8 min-w-9 items-center justify-center bg-white text-xs font-black text-[#7a45aa]">
                  ...
                  <button
                    ...
                    className="inline-flex min-w-28 items-center justify-center rounded-full bg-white px-4 py-1.5 text-base font-black uppercase tracking-[0.06em] text-(--orchird-green) transition hover:bg-(--orchird-green) hover:text-white"
                  >
                    Comprar
                  </button>
                </div>
